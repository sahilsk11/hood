...
def parse_row(row):
    ...
    if len(row) > 10 and ("LIMIT" in header or "STOP LOSS" in header):
        ...
        row.pop(10)
    header = (
        header.replace(" STOP LOSS", "")
        .replace(" TRAILING STOP", "")
        .replace(" LIMIT", "")
    )
    print(row)
    ...
    if "BUY" in header:
        ...
        if (
            asset_name == "ETHEREUM"
            or asset_name == "BITCOIN"
            or asset_name == "DOGECOIN"
        ):
            long_date = (
                row[13]
                .replace("2022,", "2022")
        ...
        if long_date == None:
            print(
                "long date missing. you need to expand the stock details then copy it over"
            )
            print(out)
            exit(1)
    ...
    for trade in date_sorted_trades:
        ...
        if "action" in trade and trade["action"] == "BUY":
            if asset not in lots:
                lots[asset] = {"total_quantity": Decimal(0), "open_lots": []}
...
def add_breakeven_percentages(lots):
    for symbol in lots.keys():
        ...
        current_price = get_price(symbol)
        gains = (
            lots[symbol]["unrealized_st_gains"] + lots[symbol]["unrealized_lt_gains"]
        )
        lots[symbol]["risk_percentage"] = calculate_breakeven_price_change(
            gains, quantity, current_price
        )
    return lots
